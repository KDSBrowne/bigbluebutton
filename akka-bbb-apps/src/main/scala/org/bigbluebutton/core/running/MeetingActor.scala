...
class MeetingActor(
  ...
  private def handleMessageThatAffectsInactivity(msg: BbbCommonEnvCoreMsg): Unit = {
    ...
    msg.core match {
      ...
      case m: SetUserTalkingReqMsg =>
        updateUserLastActivity(m.header.userId)
        handleSetUserTalkingReqMsg(m)
      ...
      case m: SetPageWhiteboardVisionPubMsg                    => state = presentationPodsApp.handle(m, state, liveMeeting, msgBus)
      case m: SetPageSelectedUserPubMsg                        => state = presentationPodsApp.handle(m, state, liveMeeting, msgBus)
      case m: SetPageUserSharedWithAllPubMsg                   => state = presentationPodsApp.handle(m, state, liveMeeting, msgBus)
